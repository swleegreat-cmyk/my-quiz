...
with st.sidebar:
    ...
    st.divider()
    st.info("💡 빅 시드 마케팅은 '안정성'을, 유명인 마케팅은 '운'을 상징합니다.")

# 3. 메인 탭 구성
tab1, tab2 = st.tabs(["📊 전략별 확산 시뮬레이션", "✍️ 7단계 핵심 퀴즈"])

# --- 탭 1: 전략 비교 시뮬레이터 (유명인 속도 하향 조정 버전) ---
with tab1:
    st.header("📈 마케팅 전략별 확산 비교")
    st.write("지문의 핵심: '유명인의 파급력'보다 '초기 시드의 규모'가 더 중요할 수 있습니다.")
    ...
    with col1:
        ...
        # 전략 A: 빅 시드 마케팅 (안정적 모델)
        st.markdown("### **[전략 A] 빅 시드 마케팅**")
        a_seed = st.number_input("초기 시드 (보통 사람 1,000명):", value=1000)
        a_r = st.slider("재생산 지수 (평범한 전파력):", 0.5, 2.0, 1.1, 0.1, key="a_r")

        st.divider()

        # 전략 B: 유명인 보증 마케팅 (위험한 모델)
        st.markdown("### **[전략 B] 유명인 보증 마케팅**")
        b_seed = st.number_input("초기 시드 (유명인 1명):", value=1)
        # 기본 R값을 2.0으로 낮추어, 혼자서는 큰 규모를 이기기 어렵게 설정
        b_r = st.slider("재생산 지수 (유명인의 전파력):", 1.0, 5.0, 2.0, 0.5, key="b_r")
        
        steps = st.slider("확산 단계 (Generation):", 1, 10, 8)
    ...
    df_compare = pd.DataFrame({
        "단계": gens,
        "빅 시드 마케팅 (다수의 보통 사람)": a_growth,
        "유명인 마케팅 (소수의 스타)": b_growth
    }).set_index("단계")

    with col2:
        st.subheader("📊 전략별 도달 범위 비교")
        st.line_chart(df_compare)
        
        m1, m2 = st.columns(2)
        m1.metric("빅 시드 최종 도달", f"{a_growth[-1]:,} 명", "안정적 확산")
        m2.metric("유명인 최종 도달", f"{b_growth[-1]:,} 명", f"R={b_r}의 한계", delta_color="inverse")
        
        st.warning(f"**학습 포인트:** 유명인의 전파력($R$)이 아무리 높더라도, 시작점이 단 1명이라면 "
                   f"1,000명으로 시작하는 빅 시드 마케팅을 따라잡기 위해서는 엄청난 시간과 운이 필요합니다.")
...
with tab2:
    ...
    if submitted:
        if not user_name:
            ...
            st.code(result_text)
            st.info("⬆️ 위 회색 박스 내용을 복사해서 선생님께 보내주세요!")
